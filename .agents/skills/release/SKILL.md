...
4. **README 版本号自动同步**
   - `release:prepare` 和 `release:auto` 脚本会自动将 `README.md` 中的 `当前版本` 和 `docs/README.en.md` 中的 `Current version` 更新为目标版本号，并将 README 变更包含在 release commit 中。
   - 无需手动更新 README 或 `git add`。
...
5. **执行本地准备**
     - 可选两条路径：
      ...
      - 预期行为：
        - 更新 `package.json` 版本号；
        - 同步 `README.md` 和 `docs/README.en.md` 中的版本号；
