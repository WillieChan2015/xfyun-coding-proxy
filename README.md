...
- **Ollama 协议兼容** — 新增 `/ollama/api/chat`、`/ollama/api/generate`、`/ollama/api/tags` 路由，自动转换 Ollama 原生协议请求为 OpenAI 格式转发，响应转换回 Ollama NDJSON 格式
- **Anthropic 协议兼容** — 新增 `/anthropic/v1/messages` 路由，透传 Anthropic Messages API 请求到讯飞 Anthropic 兼容端点，支持 Claude Code / Cursor（Anthropic 模式）等客户端
- **一键配置客户端** — 新增 `maas-coding-proxy setup` 子命令，交互式引导配置 Claude Code 等客户端使用本地代理，自动检测安装、预览变更、备份并写入配置
...
- 支持的端点：`POST /ollama/api/chat`、`POST /ollama/api/generate`、`GET /ollama/api/tags`
...
- Ollama 协议路由前缀为 `/ollama`，支持 `/api/chat`、`/api/generate`、`/api/tags` 三个端点。
- Ollama 的 `keep_alive`、`options.top_k`、`options.num_ctx` 等本地特有参数会被静默丢弃。
- Ollama 流式响应使用 NDJSON 格式（`application/x-ndjson`），与 OpenAI 的 SSE 格式不同。
- Anthropic 协议路由前缀为 `/anthropic`，支持 `/v1/messages` 端点。
- Anthropic 协议的 Extended Thinking、Vision、Tool Use 等功能均透传，不做协议转换。
- Anthropic 流式响应使用 SSE 格式（`text/event-stream`），与 Ollama 的 NDJSON 格式不同。
- `setup` 子命令支持配置 Claude Code，后续将扩展支持更多客户端；`setup restore` 支持查看与恢复备份配置。
